...
- The `moodboard` Render service tracks this repo's `main` branch with auto-deploy on commit (connected via GitHub account auth, not a bare repo URL — the earlier public-URL connection never registered a webhook, so every deploy required a manual trigger via the dashboard or API). A separate `moodboard-multi-user` service on Render deploys from the unrelated `moodboard-multi-user` repo — never target it for a change made here.
